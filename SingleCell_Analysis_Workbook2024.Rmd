...
Predict with another references dataset

```{r}

ref.ka <- readRDS(paste(file_path,"KamathTotal_downsample.RDS", sep = "/"))
colnames(ref.ka@meta.data)
Idents(ref.ka) <- "Cell_Type"

# make the refrence object into a dgCMat_ref class
dgCMat_ref.ka <- GetAssayData(ref.ka)
# get the cell type data
Idents(ref.ka) <- "Cell_Type"
ref_celltypes.ka <- Idents(ref.ka)
unique(ref_celltypes.ka)
...
scClassify_res.ka <- scClassify(exprsMat_train = dgCMat_ref.ka,
                             cellTypes_train = ref_celltypes.ka,
                             exprsMat_test = dgCMat_query,
                             cellTypes_test = query_clusters,
                             tree = "HOPACH",
                             algorithm = "WKNN",
...
```{r}
scClassify_res.ka$trainRes
plotCellTypeTree(cellTypeTree(scClassify_res.ka$trainRes))
```
...
```{r}
table(scClassify_res.ka$testRes$test$pearson_WKNN_limma$predRes,query_clusters)
